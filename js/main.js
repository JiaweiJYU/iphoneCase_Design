...
const DEVICE_PARAMS = {
    'iphone-11': withOverrides({
        ...
        holes: [
            // 单矩形开孔（示例）
            { type: 'rect', x: 300, y: 90, w: 90, h: 120, r: 16 }
        ]
    }),
    'iphone-14-pro': withOverrides({
        ...
        holes: [
            // 统一把多摄像头区域整体挖空
            { type: 'rect', x: 292, y: 84, w: 110, h: 130, r: 18 }
        ]
    }),
    'iphone-15-pro': withOverrides({
        print: { x: 30, y: 24, w: 360, h: 770, r: 44 },
        holes: [
            { type: 'rect', x: 295, y: 86, w: 112, h: 132, r: 18 }
        ]
    })
};
...
function exportPNG() {
    ...
    for (const h of state.cameraCutouts) {
        if (h.type === 'circle') {
            ...
            oc.beginPath();
            oc.arc((h.x - state.caseArea.x) * scale,
                (h.y - state.caseArea.y) * scale,
                h.r * scale, 0, Math.PI * 2);
            oc.fill();
            oc.restore();
        } else if (h.type === 'rect') {
            oc.save();
            ...
            oc.beginPath();
            oc.moveTo(x + r, y);
            oc.arcTo(x + w, y, x + w, y + hgt, r);
            oc.arcTo(x + w, y + hgt, x, y + hgt, r);
            oc.arcTo(x, y + hgt, x, y, r);
            oc.arcTo(x, y, x + w, y, r);
            oc.closePath();
            oc.fill();
            oc.restore();
