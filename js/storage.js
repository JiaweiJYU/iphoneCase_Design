window.StorageIO = (function(){
  const { state } = State;
  function install(ctx){
    document.getElementById('btnClear').addEventListener('click', ()=>{
      state.objects = []; state.selectedId = null; Render.draw(ctx);
    });
      ...
      input.onchange = ()=>{
        ...
        fr.onload = ()=>{
          try{
            ...
            state.objects = [];
            if(list.length===0){ Render.draw(ctx); return; }
            let loaded = 0;
            list.forEach(item=>{
              const img = new Image();
              img.onload = ()=>{
                state.objects.push({...item, img});
                loaded++; if(loaded===list.length){ Render.draw(ctx); }
              };
              img.src = item.src;
            });
